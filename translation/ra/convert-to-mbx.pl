...
if ($commands ne "" || $macrosextra ne "") {
	print $out "  <macros>\n";
	print $out xml_escape_text($commands);
	print $out xml_escape_text($macrosextra);
	print $out "  </macros>\n";
}
	...
	while(1) {
		...
		if ( ! defined $line) {
			...
			if (@ins) {
				...
				# A final \end{document} need not be followed by a blank line.
				# In that valid case the accumulated paragraph already contains it.
				if ($para =~ m/\\end\{document\}[ \t\r\n]*$/) {
					print "END OF MAIN FILE AFTER \\end{document}\n))))\n\n";
					last;
				}
				# This should not happen: no end-of-document marker was read.
	...
	#FIXME: no notation index in HTML version yet
		...
		if ($para =~ s/^(.*?)\\end\{multline\}[ \n]*//s) {
			...
			if ($theid eq "") {
				...
				print $out "<md xml:id=\"$theid\" number=\"yes\" eqnumber=\"$the_num\" latexenv=\"multline\">$indexes\n";
			}
			print "EQ(multline) = $eqn\n";
			print $out "$eqn";
			print $out "</md>\n";
		...
		if ($para =~ s/^(.*?)\\end\{equation\}[ \n]*//s) {
			...
			if ($theid eq "") {
				...
				print $out "<md xml:id=\"$theid\" number=\"yes\" eqnumber=\"$the_num\">$indexes\n";
			}
			print "EQ = $eqn\n";
			print $out "$eqn</md>\n";
		} else {
